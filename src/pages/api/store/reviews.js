...
export default async function handler(req, res) {
  ...
  const { accessToken: explicitAccessToken = "", storeUrl = "", page = 1 } = req.body || {};
  ...
  try {
    const response = await getStoreJsonWithAuth({
      endpoint: `${SHOPDIBZ_URLS.store}${storeUrl}/reviews/`,
      accessToken,
      query: { page },
    });
    ...
    res.status(500).json({
      message: error instanceof Error ? error.message : "Reviews request failed",
    });
  }
}
